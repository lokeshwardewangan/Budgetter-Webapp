...
export const create = asyncHandler(async (req, res) => {
  const data = await lentMoneyService.addEntry(req.userId as string, req.body);
  res
...
export const list = asyncHandler(async (req, res) => {
  const entries = await lentMoneyService.listEntries(req.userId as string);
  res
...
export const receive = asyncHandler(async (req, res) => {
  const data = await lentMoneyService.markReceived(req.userId as string, req.params.id as string);
  res
    .status(StatusCodes.OK)
